...
const   https = require('https'), zlib = require('zlib'), fs = require('fs'), env = process.env;
...
function main() {
    ...
    let prefix = env.INPUT_PREFIX ? `${env.INPUT_PREFIX}-` : '';
    if (prefix.startsWith("refs/")) {
        prefix = prefix.replace("refs", "pr");
    }
